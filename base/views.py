...
def mapping_page(request, session_id):
    ...
    if column_count > 5:
        ...
        sheet_info["too_many_columns_count"] = column_count
        # رفع خطا: اطمینان از اینکه warning رشته است (None -> "")
        current_warning = sheet_info.get("warning") or ""
        sheet_info["warning"] = current_warning + f" | فایل دارای {column_count} ستون است. برای سهولت، لطفاً ستون‌های مهم (amount, date, customer, product, status) را انتخاب کنید."
    else:
        sheet_info["too_many_columns"] = False


    context = {
        'session_id': session_id,
        'columns': df.columns.tolist(),  # بدون clean_column_name اضافی
        'suggested_mapping': suggested,
